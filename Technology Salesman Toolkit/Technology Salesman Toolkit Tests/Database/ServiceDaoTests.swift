...
class ServiceDaoTests: XCTestCase {
    ...
    func testAddThreeServices_DatabaseContainsThreeServices() {
        let realm = DatabaseTestUtils.getRealm()
        var realmServices = realm.objects(Service.self)
        
        XCTAssertEqual(realmServices.count, 0)
        ...
        try! realm.write {
            realm.delete(realm.objects(Service.self))
            realm.add(services)
        }
        
        realmServices = realm.objects(Service.self)
        
        XCTAssertEqual(realmServices.count, 3)
    }
    ...
    func testAddThreeServices_addTwoServices_DatabaseContainsTwoServices() {
        ...
        XCTAssertEqual(realmServices.count, 2)
    }
